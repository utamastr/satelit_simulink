function [sys,x0,str,ts] = batang_animasi(t,x,u,flag,Config)
%Animasi Batang S-Function for displaying 6DoF trajectories
%
...
% Modified by: Ridanto
% $Revision: 1.2.2.6 $  $Date: 2007/11/16 23:11:28 $
...
%
   [xm,ym,zm] = miss_shape;
   handle.craft = patch(xm,ym,zm,[0 0 1]);
...
%	count=1;	     % Counter for number of individual patches
	...
	len 		= 30;				% Total Length (no units)
	radius		= 10;			% Radius of body
 	...
 	rad_throt	= 1.3/2;			% radius of motor exit
   	...
   	c_g 			= 15;			% Position of c.g. w.r.t nose
...
% Wing shapes 
%
	%xcraft{count}=[8 8 15 15]-c_g-5;
	%ycraft{count}=[-zc_range(1) -zc_range(1)+10 -zc_range(1)+10 -zc_range(1)];
